...
-- 8. ejecutar luego de hacer subscriptor
...
-- 16 crear limon como distribuidor
...
-- 17. crear publicacion meidnte wizard


-- 18 CREAR SUSCERIPTOR Corporativo
...
UPDATE Warehouse.StockItemHoldings
SET QuantityOnHand = 175621
WHERE StockItemID = 1




CREATE OR ALTER PROCEDURE [dbo].[sp_MSdel_SalesInvoices]
        @c1 int,
        @c2 int,
        @c3 int,
        @c4 int,
...
BEGIN
    ...
    -- CustomerTransactions (si aplica)
    DELETE FROM Sales.CustomerTransactions
    WHERE InvoiceID = @c1;

    -- StockItemTransactions (si aplica)
    DELETE FROM Warehouse.StockItemTransactions
    WHERE InvoiceID = @c1;
    ...
    IF @@ROWCOUNT = 0
        ...
        BEGIN
            ...
            BEGIN
                SET @primarykey_text = @primarykey_text + '[InvoiceID] = ' 
                    + CONVERT(nvarchar(100), @c1, 1)

                EXEC sp_MSreplraiserror 
                    @errorid = 20598, 
                    @param1  = N'[Sales].[Invoices]', 
                    @param2  = @primarykey_text, 
                    @param3  = 13234 
            END
            ELSE
...
EXEC dbo.sp_MSdel_SalesInvoices
    @c1 = 1,
